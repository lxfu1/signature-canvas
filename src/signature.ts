...
class Signature {
  ...
  constructor(config: IConfig) {
    ...
    if (deafultWidth < minWidth || deafultWidth > maxWidth) {
      console.warn('deafultWidth should be between minWidth and maxWidth');
    }
    this.width = width;
    this.height = height;
    ...
    this.deafultWidth = deafultWidth;
     ...
     */
    this.reduceFactor = 1 / (this.maxDistance * (maxWidth - minWidth));
    /**
     * 递增速率不高于0.1
     * (max - min ) * minDistance * base <= 0.1, minDistance为16ms内用户划过的最小距离，可以调整。
     */
    this.addFactor = 1 / (10 * (maxWidth - minWidth) * this.minDistance);
  ...
  private getLineWidth = (endX: number, endY: number) => {
    ...
    if (d < Math.min(minDistance, currentDistance)) {
      lineWidth += (maxWidth - currentLineWidth) * (minDistance - d) * addFactor;
      lineWidth = Math.min(maxWidth, lineWidth);
    }
    if (d > Math.max(minDistance, currentDistance)) {
  ...
  private updateStroke = (e: MouseEvent | Touch) => {
    const { context, penColor, windowToCanvas, lastLocation, getLineWidth } = this;
  ...
  private drawBackground = () => {
    const { context, getWidth, getHeight, backgroundColor, backgroundImage } = this;
    ...
    if (backgroundImage?.src) {
      ...
      img.onload = () => {
        context.drawImage(img, backgroundImage?.x || 0, backgroundImage?.y || 0);
      };
    }
  };
  ...
  private drawImage = data => {
    const { context, clear, width, height } = this;
    const img = new Image();
    img.src = data;
    img.onload = () => {
      clear(false);
  ...
  initData = (data: string) => {
    if (data.indexOf('data:image/png;base64') === -1) {
      console.warn('The data format is not incorrect, please use Base64');
      return;
    }
    this.drawImage(data);
  ...
  /* 撤销重做 */
  undo = () => {
    const { canvasHistory, context, getWidth, getHeight, drawBackground } = this;
    ...
    if (canvasHistory.length === 0) {
      ...
      this.drawImage(canvasHistory[canvasHistory.length - 1]);
    }
  };
